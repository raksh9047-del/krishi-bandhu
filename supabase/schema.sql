...
-- Seed the 13 crops / 10 mandis (matches constants.ts exactly).
...
create index idx_parchi_farmer_mandi_ts on parchi_ledger (farmer_id, mandi_id, "timestamp");
create index idx_parchi_crop on parchi_ledger (crop_id);
create index idx_parchi_mandi on parchi_ledger (mandi_id);
create index idx_parchi_trader on parchi_ledger (trader_id);

-- ─────────────────────────── sowing_signals ───────────────────────────
create table sowing_signals (
  crop_id text not null references crops(id) on delete restrict,
  mandi_id text not null references mandis(id) on delete restrict,
...
-- ─────────────────────────── fpo_price_entries ───────────────────────────
create table fpo_price_entries (
  id uuid primary key default gen_random_uuid(),
...
create index idx_fpo_price_crop_mandi on fpo_price_entries (crop_id, mandi_id);
create index idx_fpo_price_entered_at on fpo_price_entries (entered_at);

-- ─────────────────────────── fpo_survey_responses ───────────────────────────
create table fpo_survey_responses (
  id uuid primary key default gen_random_uuid(),
  fpo_id uuid not null references users(id) on delete restrict,
...
create index idx_survey_crop on fpo_survey_responses (crop_id);
...
create index idx_disputes_parchi on disputes (parchi_id);
...
create index idx_storage_district on storage_directory (district);

-- ─────────────────────────── backhaul_trucks ───────────────────────────
create table backhaul_trucks (
  id uuid primary key default gen_random_uuid(),
...
create index idx_backhaul_mandi on backhaul_trucks (from_mandi_id);
...
create policy "backhaul_fpo_insert" on backhaul_trucks
  for insert to authenticated
  with check (true); -- refine to a specific fpo_id column if you add one later
